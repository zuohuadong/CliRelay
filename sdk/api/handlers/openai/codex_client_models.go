...
func (h *OpenAIAPIHandler) codexClientModelsResponse(models ...[]map[string]any) map[string]any {
	selectedModels := []map[string]any(nil)
	if len(models) > 0 {
		selectedModels = models[0]
	} else if h != nil {
		selectedModels = h.Models()
	}
	optimizeMultiAgentV2 := h != nil && h.Cfg != nil && h.Cfg.CodexOptimizeMultiAgentV2
	return codexmodels.BuildResponse(selectedModels, registry.GetGlobalRegistry().GetModelProviders, optimizeMultiAgentV2)
}
