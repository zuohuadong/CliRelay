...
type Manager struct {
	...
	homeSessionLifecycleMu sync.RWMutex
	...
	go func() {
		...
		emit := func(chunk cliproxyexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				...
				m.recordExecutionResult(ctx, Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: false, Error: rerr}, auth, request.ephemeral)
			}
			if !forward {
				return false
			}
		...
		if !failed {
			m.recordExecutionResult(ctx, Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: true}, auth, request.ephemeral)
		}
	}()
...
func (m *Manager) Execute(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	if lastErr != nil {
		if hasAntigravityProvider(normalized) && shouldAttemptAntigravityCreditsFallback(m, lastErr, normalized) {
			if resp, ok, errFallback := m.tryAntigravityCreditsExecute(ctx, req, opts); errFallback != nil {
...
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	if lastErr != nil {
		if hasAntigravityProvider(normalized) && shouldAttemptAntigravityCreditsFallback(m, lastErr, normalized) {
			if result, ok, errFallback := m.tryAntigravityCreditsExecuteStream(ctx, req, opts); errFallback != nil {
...
func (m *Manager) executeHome(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, countTokens bool) (cliproxyexecutor.Response, error) {
	...
	for homeAuthCount := 1; ; homeAuthCount++ {
		...
		models, _, pooled, aliasResult := m.preparedExecutionModelsWithAlias(auth, routeModel, opts)
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		if errPrepare != nil {
			if selection == nil && responseAffinityActive(opts) {
				return nil, previousResponseReplayError(req, opts)
			}
			if selection == nil && isTerminalEgressError(errPrepare) {
				if isCredentialLocalEgressFallbackError(errPrepare) {
					opts = withoutPinnedAuth(opts)
					lastErr = errPrepare
					if homeMode {
...
// reportHomeResult observes a Home dispatch result without updating local auth state.
...
func (m *Manager) CloseExecutionSession(sessionID string) {
	...
	if sessionID == CloseAllExecutionSessionsID {
		m.homeSessionLifecycleMu.Lock()
		unlockSession = m.homeSessionLifecycleMu.Unlock
	} else {
		m.homeSessionLifecycleMu.RLock()
		lock, _ := m.homeSessionLocks.LoadOrStore(sessionID, &sync.Mutex{})
		mutex, ok := lock.(*sync.Mutex)
		if !ok || mutex == nil {
			m.homeSessionLifecycleMu.RUnlock()
			return
		}
		mutex.Lock()
		unlockSession = func() {
...
func (m *Manager) persist(ctx context.Context, auth *Auth) error {
	...
	if IsPluginVirtualAuth(auth) || IsFileBundleAuth(auth) {
		return nil
	}
