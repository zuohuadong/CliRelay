...
func (h *OpenAIAPIHandler) codexClientModelsResponse(models []map[string]any) map[string]any {
	return codexClientModelsResponse(models, registry.GetGlobalRegistry().GetModelProviders)
}
...
func applyCodexClientModelMetadata(entry map[string]any, id string, model map[string]any) {
	...
	if info != nil {
		...
		if info.Type == registry.OpenAIImageModelType || info.Type == registry.OpenAIVideoModelType {
