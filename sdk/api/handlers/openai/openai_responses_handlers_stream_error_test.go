...
func TestForwardResponsesStreamOverloadedErrorWritesFailed(t *testing.T) {
	gin.SetMode(gin.TestMode)
	base := handlers.NewBaseAPIHandlers(&sdkconfig.SDKConfig{}, nil)
	h := NewOpenAIResponsesAPIHandler(base)
	...
	errs <- &interfaces.ErrorMessage{
		...
		Error:      errors.New(`{"error":{"code":"server_is_overloaded","type":"service_unavailable_error","message":"Our servers are currently overloaded. Please try again later."}}`),
	}
	close(errs)
	...
	if !strings.Contains(body, `"code":"server_is_overloaded"`) {
		t.Fatalf("expected server_is_overloaded code, got: %q", body)
	}
	if strings.Contains(body, "event: response.completed") || strings.Contains(body, "event: response.output_item.done") {
		t.Fatalf("overload must not synthesize completed output, got: %q", body)
	}
}
