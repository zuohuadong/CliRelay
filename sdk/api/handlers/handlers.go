...
func (h *BaseAPIHandler) executeWithAuthManagerFormats(ctx context.Context, entryProtocol, exitProtocol, modelName string, rawJSON []byte, alt string, allowImageModel bool, execOptions modelExecutionOptions) ([]byte, http.Header, *interfaces.ErrorMessage) {
	...
	setEstimatedInputTokensMetadata(reqMeta, normalizedModel, rawJSON)
...
func (h *BaseAPIHandler) executeStreamWithAuthManagerFormats(ctx context.Context, entryProtocol, exitProtocol, modelName string, rawJSON []byte, alt string, allowImageModel bool, execOptions modelExecutionOptions) (<-chan []byte, http.Header, <-chan *interfaces.ErrorMessage) {
	...
	interceptorHost := h.interceptorHost()
	streamInterceptorsActive := streamInterceptorsEnabled(interceptorHost)
	// HTTP callers must finish bootstrap recovery and header interception before
	// this method returns. WebSocket callers have already upgraded and therefore
	// keep their existing asynchronous bootstrap path.
	rawStreamHeaders := cloneHeader(streamResult.Headers)
	baseStreamHeaders := cloneHeader(streamResult.Headers)
	chunks := streamResult.Chunks
	if chunks == nil {
	...
	readInitialStreamChunks := func() {
		for {
			...
			payload, deliverable, errTransform := transformStreamPayload(chunk.Payload, &bootstrapChunkIndex, bootstrapHistoryChunks)
			if errTransform != nil {
				bootstrapErr = errTransform
				return
			}
			if !deliverable {
				continue
	...
	if !downstreamWebsocket {
		for bootstrapRetry := 0; !streamCanceledBeforeRead; {
			readInitialStreamChunks()
			if streamCanceledBeforeRead || bootstrapErr != nil || bootstrapStreamErr == nil {
				break
			}
			if bootstrapRetry >= maxBootstrapRetries || !streamBootstrapRetryEligible(bootstrapStreamErr) {
	...
	outer:
		for {
			...
			if chunk.Err != nil {
				streamErr := chunk.Err
				if downstreamWebsocket && !sentPayload && asyncBootstrapRetries < maxBootstrapRetries && streamBootstrapRetryEligible(streamErr) {
					asyncBootstrapRetries++
					retryResult, retryErr := h.AuthManager.ExecuteStream(ctx, providers, req, opts)
					...
					if retryErr != nil {
						...
						streamErr = fmt.Errorf("auth manager returned nil stream")
					}
				}
			...
			payload, deliverable, errTransform := transformStreamPayload(chunk.Payload, &chunkIndex, historyChunks)
			if errTransform != nil {
				_ = sendErr(errTransform)
				return
			}
			if !deliverable {
				continue
