...
// NextProxyPoolURL returns an enabled proxy-pool URL using round-robin selection.
func (cfg *Config) NextProxyPoolURL() string {
	if cfg == nil || len(cfg.ProxyPool) == 0 {
		return ""
	}
	enabled := make([]ProxyPoolEntry, 0, len(cfg.ProxyPool))
	for _, entry := range cfg.ProxyPool {
	...
	idx := atomic.AddUint64(&proxyPoolCounter, 1)
	return strings.TrimSpace(enabled[int(idx-1)%len(enabled)].URL)
}
