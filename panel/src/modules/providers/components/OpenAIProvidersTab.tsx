...
interface OpenAIProvidersTabProps {
  ...
  onToggleProviderResponseEndpoint?: (providerIndex: number, enabled: boolean) => void;
  onToggleKeyEntryEnabled?: (providerIndex: number, entryIndex: number, enabled: boolean) => void;
  showResponseEndpointToggle?: boolean;
...
export function OpenAIProvidersTab({
  ...
  onToggleProviderResponseEndpoint,
  onToggleKeyEntryEnabled,
  showResponseEndpointToggle = false,
        ...
        >
          {providers.map((provider, idx) => {
            ...
            const responseEndpointEnabled =
              responseEndpointLocked || provider.responseEndpoint === true;
            ...
            return (
              ...
              >
                <div className="space-y-3">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex w-full flex-wrap items-center justify-end gap-2 sm:w-auto">
                      {showResponseEndpointToggle ? (
                        <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 px-2 py-1 dark:border-neutral-800 dark:bg-neutral-950/70">
                          <span
                            className="whitespace-nowrap text-[11px] font-semibold text-slate-600 dark:text-white/65"
                            title={t("providers.response_endpoint")}
                          >
                            {t("providers.response_endpoint_short")}
                          </span>
                          <ToggleSwitch
                            checked={responseEndpointEnabled}
                            disabled={responseEndpointLocked || !onToggleProviderResponseEndpoint}
                            ariaLabel={`${t("providers.response_endpoint")} ${provider.name}`}
                            onCheckedChange={(enabled) =>
                              onToggleProviderResponseEndpoint?.(idx, enabled)
                            }
                          />
                        </div>
                      ) : null}
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600 dark:text-white/65 tabular-nums">
                    <span>
