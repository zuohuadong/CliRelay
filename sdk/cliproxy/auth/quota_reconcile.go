...
func (m *Manager) shouldProbeQuota(auth *Auth, now time.Time) bool {
	if m == nil || auth == nil || auth.Disabled || strings.TrimSpace(auth.ID) == "" {
		return false
	}
...
func applyQuotaProbeResult(auth *Auth, result *QuotaProbeResult, now time.Time) (bool, []string) {
	...
	if len(auth.ModelStates) > 0 {
		...
		if auth.Status != beforeStatus || auth.StatusMessage != beforeStatusMessage || !quotaErrorsEqual(auth.LastError, beforeLastError) {
			changed = true
		}
...
func quotaErrorsEqual(a, b *Error) bool {
	if a == nil || b == nil {
		return a == nil && b == nil
	}
