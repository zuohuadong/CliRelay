...
func appendPendingResponsesDoneEvents(out *[][]byte, st *oaiToResponsesState, nextSeq func() int) {
	if len(st.MsgItemAdded) > 0 {
...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	emitToolItem := func(key string, force bool) {
		...
		if callID == "" {
			responseID := st.SourceResponseID
			if responseID == "" {
				responseID = strings.TrimPrefix(st.ResponseID, "resp_")
			}
			callID = fmt.Sprintf("call_%s_%s", responseID, strings.ReplaceAll(key, ":", "_"))
	...
	if !st.Started {
		st.SourceResponseID = root.Get("id").String()
		st.ResponseID = st.SourceResponseID
				...
				if tcs := msg.Get("tool_calls"); tcs.Exists() && tcs.IsArray() {
					...
					tcs.ForEach(func(tcIndex, tc gjson.Result) bool {
						...
						if callID == "" {
							...
							callID = fmt.Sprintf("call_%s_%d_%d", sourceID, choice.Get("index").Int(), tcIndex.Int())
						}
