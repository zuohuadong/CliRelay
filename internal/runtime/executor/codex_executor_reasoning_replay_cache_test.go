...
func TestCodexExecutorExecuteStreamRetriesWithoutClientReasoningEncryptedContentOnInvalidSignature(t *testing.T) {
	internalcache.ClearCodexReasoningReplayCache()
	t.Cleanup(internalcache.ClearCodexReasoningReplayCache)

	staleEncryptedContent := validCodexReasoningEncryptedContentForTestSeed(16)
	var bodies [][]byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, errRead := io.ReadAll(r.Body)
		if errRead != nil {
	...
	executor := NewCodexExecutor(&config.Config{})
	streamResult, err := executor.ExecuteStream(context.Background(), &cliproxyauth.Auth{
		ID: "auth-replay-switch-model-stream",
		Attributes: map[string]string{
			"base_url": server.URL,
			"api_key":  "test",
		},
		...
		Payload: []byte(`{
			"model":"gpt-5.5",
			"metadata":{"user_id":"{\"device_id\":\"device-test\",\"account_uuid\":\"\",\"session_id\":\"session-switch-model\"}"},
			"messages":[
	...
	for chunk := range streamResult.Chunks {
		if chunk.Err != nil {
			t.Fatalf("unexpected stream chunk error after invalid signature retry: %v", chunk.Err)
		}
	}
	if len(bodies) != 2 {
		t.Fatalf("upstream request count = %d, want 2", len(bodies))
