...
func TestCloseExecutionSessionSerializesWithHomeSessionLock(t *testing.T) {
	manager := NewManager(nil, nil, nil)
	ctx := cliproxyexecutor.WithDownstreamWebsocket(context.Background())
	opts := cliproxyexecutor.Options{Metadata: map[string]any{
	...
	lockBefore, ok := manager.homeSessionLocks.Load("reclaim-lock")
	if !ok {
	...
	select {
	case <-closed:
		t.Fatal("CloseExecutionSession() bypassed the active session lock")
	...
	case <-time.After(time.Second):
		t.Fatal("CloseExecutionSession() did not finish after session unlock")
	}
	lockAfter, ok := manager.homeSessionLocks.Load("reclaim-lock")
	if !ok || lockAfter != lockBefore {
		t.Fatal("session close replaced the mutex and allowed split locking")
	}
}
