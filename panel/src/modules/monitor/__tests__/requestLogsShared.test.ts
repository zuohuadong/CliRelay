...
describe("requestLogsShared", () => {
  ...
  test("deduplicates system call filter options", () => {
    const options = buildRequestLogKeyOptions(
      ...
      { allKeys: "全部密钥", systemCall: "系统调用", unnamedKey: "未命名" },
    );
  ...
  test("does not expose raw key text when a request-log key name is missing", () => {
    const options = buildRequestLogKeyOptions(
      ["sk-live-123456"],
      {},
      { allKeys: "全部密钥", systemCall: "系统调用", unnamedKey: "未命名" },
    ...
    expect(options.find((option) => option.value === "sk-live-123456")?.label).toBe("未命名");
  });
